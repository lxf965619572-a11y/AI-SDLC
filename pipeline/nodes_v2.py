"""重构后的 nodes.py：使用统一状态管理器。

主要改进：
1. 使用 StateManager 进行状态转换
2. 移除直接操作 Project.status 的代码
3. 更清晰的职责分离
"""
import json
import traceback
...
def parse_input(state: PipelineState) -> dict:
    ...
    from parsing.extractor_v2 import extract_structured, render_markdown

    project_id = state["project_id"]
    state_mgr = get_state_manager()

    with SessionLocal() as session:
        # 状态转换（如果还不是 PARSING 状态）
        current_state = state_mgr.load_state(session, project_id)
        if current_state.status != ProjectStatus.PARSING:
            state_mgr.transition(
                session, project_id,
                ProjectStatus.PARSING,
                stage=StageType.PARSE,
                log_message="开始解析上传文档"
            )
    ...
    def agent_node(state: PipelineState) -> dict:
        ...
        with SessionLocal() as session:
            ...
            state_mgr.transition(
                ...
                log_message=f"{'带评审意见修订' if retry_comments else '生成'}「{STAGE_TITLES[stage]}」"
            )
            ...
            # 保存到规范化表
            stage_agents.save_to_normalized_tables(session, art_id, stage, meta)

            log(session, project_id, f"「{STAGE_TITLES[stage]}」v{version} 生成完成", stage)

            # 状态转换到 WAITING_REVIEW
            state_mgr.transition(
                session, project_id,
                ProjectStatus.WAITING_REVIEW,
                stage=StageType(stage),
                log_message=f"「{STAGE_TITLES[stage]}」等待人工评审"
            )
